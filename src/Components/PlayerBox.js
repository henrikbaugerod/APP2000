...
const PlayerBox = (props) => {
  const setPlayerId = (id) => {
    ...
    const playerPressed = sessionStorage.getItem("playerPressed");

    if (playerPressed == "1") {
      sessionStorage.setItem("playerId1", props.id);
    } else if (playerPressed == "2") {
      sessionStorage.setItem("playerId2", props.id);
    } else {
      console.log("Feil");
    }
    ...
    if (props.checkbox) {
      ...
      if (playerBoxImgSrc === "./images/plus-solid.svg") {
        ...
        const tempArray = props.registeredPlayers.filter(
          (index) => index !== props.id
        );
  ...
  return (
    <button
      className="col-12 mb-2 text-white border-0 playerButton px-0"
      key="{props.place}"
      style={{ backgroundColor: "transparent" }}
      onClick={() => setPlayerId(props.id)}
    >
      <div
        className={`bg-purple p-3 playerBox rounded-3 position-relative`}
        id={`box-${props.id}`}
      >
